import SimplrRouterStyler from './simplr-router-styler.js';
import { TransitionDirection } from './simplr-router-transition-direction.js';
import { TransitionSpeed } from './simplr-router-transition-speed.js';
...
export default class SimplrRouter {
    ...
    async _handleElementAddedToDomActions(newView) {
        ...
        if (
            ...
            previousView.wrapper.classList &&
            !this.stackedViews
        ) {
        ...
        target.addEventListener('click', e => {
            ...
            if (eventTarget.localName !== 'a') {
                for (let item of e.path) {
                    ...
                    if (item.dataset.hasOwnProperty('simplrRoute')) {
                        eventTarget = item;
                        break;
                    }
                }
            }
            if (eventTarget.dataset.hasOwnProperty('simplrRoute')) {
                e.preventDefault();
                this._goToView(eventTarget.getAttribute('href'));
            }
        });
    }
    ...
    async _goToView(view) {
        ...
        let transitionSuccessful = await this._transitionToView(anchorAddress);
    ...
    async _transitionToView(viewToTransitionTo, isPreviousAction) {
    ...
    setForbiddenAction(action) {
        this.forbiddenAction = action;
    }

    setNotFoundAction(action) {
