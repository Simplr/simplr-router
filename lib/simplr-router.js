...
export default class SimplrRouter {
    ...
    async _handleElementAddedToDomActions(newView) {
        ...
        if (previousView && previousView.wrapper) {
    ...
    _goToPreviousPage() {
        ...
        let oppositeTransitionDirection = this._getOppositeTransitionAttribute();
        oldActiveView.wrapper.setAttribute(this.transitionDirection, '');
        newView.wrapper.setAttribute(oppositeTransitionDirection, '');

        window.requestAnimationFrame(() => {
            newView.wrapper.removeAttribute(oppositeTransitionDirection);
        });
    }
    ...
    _updateActiveView(route, wrapper) {
        this.lastActiveView = this.activeView.parentNode.className.includes('simplr-router-view-wrapper')
