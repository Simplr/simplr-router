...
export default class SimplrRouter {
    ...
    constructor(activeView, routes, useStyles) {
        this.rootView = activeView;
        this.activeView = activeView;
        if (activeView == null) {
            throw Error("Cannot initialize SimplrRouter because active view is null.");
        }
    ...
    static init(activeView, routes, useStyles) {
        console.info("Initializing Simplr Router...");
        SimplrRouter.Router = new SimplrRouter(activeView, routes, useStyles);
    }
    ...
    _loadRoutes(routes) {
        if (!routes) {
            throw Error("No routes passed during initialization. \n\n" +
    ...
    static setTransitionSpeed(speed) {
        if (typeof speed == "number" || (!isNaN(Number(speed)) && typeof Number(speed) == "number")) {
            this.Router.transitionSpeed = speed;
            this.Router._createStyles();
        } else {
            throw Error(`Attempted to set transition speed to ${speed}, but ${speed} is not a viable speed parameter.`);
        }
    }
    ...
    static setTransitionDirection(transitionDir) {
        if (Object.values(TransitionDirection).includes(transitionDir)) {
            ...
            throw Error(`Attempted to set transition direction to ${transitionDir}, but ${transitionDir} is not a preset direction.\nPlease use a preset direction spesified in TransitionDirection-object.`);
        }
    }
