...
export default class SimplrRouter {
    ...
    _getStackedViewsFromLocalStorage() {
        ...
        for (let i = 1; i < viewStackCopy.length; i++) {
            console.log('Transitioning to view ' + i);
            ...
            this._handleElementAddedToDomActions(latestAdditionToStack.wrapper);
            latestAdditionToStack.wrapper.classList.remove(
                this.transitionDirection
            );
        }
    }
    ...
    // DEPRECATED
    // We now add the listener to the wrapper instead
