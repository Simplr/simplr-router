...
test("_loadRoutes should throw error if passed null or undefined", () => {
    SimplrRouter.init(mockActiveView(), routes, false);
    expect(() => {SimplrRouter.Router._loadRoutes(null)}).toThrowError();
    expect(() => {SimplrRouter.Router._loadRoutes(undefined)}).toThrowError();
    expect(() => {SimplrRouter.Router._loadRoutes(routes)}).not.toThrowError();
});
...
test("_mapParameters should return url parameters mapped to json object", () => {
    ...
    let mappedParams = SimplrRouter.Router._mapParameters(hrefParams, routeParams);
...
test("_replaceRouteParamsWithHrefParams should replace parameters in route to parameters from url", () => {
    ...
    let newPath = SimplrRouter.Router._replaceRouteParamsWithHrefParams(path, hrefParams, routeParams);
    expect(newPath).toBe(expectedPath);
});

test("_parseParameters should return the correct route for path", () => {
    SimplrRouter.init(mockActiveView(), routes, false);
    let route = {path: "/foo/:id/bar/:secondId", view: "test-view-with-params"};
    let hrefParams = [12, 123];
    let path = "/foo/12/bar/123";

    let matchedRoute = SimplrRouter.Router._parseParameters(route, path, hrefParams);
...
test("_parseParameters should return null if the amount of parameters in the route and the path don't match", () => {
    SimplrRouter.init(mockActiveView(), routes, false);
    let route = {path: "/foo/:id/bar/:secondId/baz/:thirdId", view: "test-view-with-params"};
    let hrefParams = [12, 123];
    let path = "/foo/12/bar/123";
...
test("_findViewForRoute should return a view if view is in routes file", () => {
    ...
    expect(SimplrRouter.Router._findViewForRoute(parametrizedRoute).path).toBe(routes[3].path.replace(":fooId", 123));
});
...
test("_updateActiveView should add active view to stack", () => {
    ...
    expect(SimplrRouter.Router.activeView).toStrictEqual(document.createElement(routes[1].view));
...
test("_handleForbiddenAction should throw error if not action or view is set", () => {
    SimplrRouter.init(mockActiveView(), routes, false);
    expect(() => {SimplrRouter.Router._handleForbiddenAction()}).toThrowError();
});
...
test("_handleNotFoundAction should throw error if not action or view is set", () => {
    SimplrRouter.init(mockActiveView(), routes, false);
    expect(() => {SimplrRouter.Router._handleNotFoundAction()}).toThrowError();
});
...
test("_transitionToView should return false if given unlisted view and not-found action is set", () => {
    SimplrRouter.init(mockActiveView(), routes, false);
    SimplrRouter.setNotFoundAction(() => {return false});
...
test("_handleBackwardsNavigation removes a view from the stack", () => {
    ...
    let viewStackSizeBeforeNavigatingBack = SimplrRouter.Router.viewStack.length;
    SimplrRouter.Router._handleBackwardsNavigation();
    expect(SimplrRouter.Router.viewStack.length).toBe(viewStackSizeBeforeNavigatingBack - 1);
});
...
test("_overrideAnchor should update anchor data attribute accordingly", () => {
    ...
    expect(anchor.dataset.hasOwnProperty("simplrRouteInitialized")).toBeTruthy();
});
...
test("_overrideAnchorActions should update all anchors data attributes accordingly", () => {
    ...
    let oldDataAnchors = Array.from(document.querySelectorAll("[data-simplr-route]"));
    let initializedDataAnchors = Array.from(document.querySelectorAll("[data-simplr-route-initialized]"));
...
test("Router transition direction should be settable to all 4 directions", () => {
    ...
    expect(() => {SimplrRouter.setTransitionDirection(TransitionDirection.LEFT)}).not.toThrowError();
    expect(() => {SimplrRouter.setTransitionDirection(TransitionDirection.RIGHT)}).not.toThrowError();
    expect(() => {SimplrRouter.setTransitionDirection(TransitionDirection.TOP)}).not.toThrowError();
    expect(() => {SimplrRouter.setTransitionDirection(TransitionDirection.BOTTOM)}).not.toThrowError();
});