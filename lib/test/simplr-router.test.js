...
test("Throws on absence of Active view as first parameter", () => {
    expect(() => {SimplrRouter.init()}).toThrowError("Cannot initialize SimplrRouter because active view is null.");
});

test("Throws on absence of routes as second parameter", () => {
    expect(() => {SimplrRouter.init(mockActiveView())}).toThrowError(/No routes passed during initialization./);
});

test("Should build router successfully", () => {
    expect(() => {SimplrRouter.init(mockActiveView(), routes, false);}).toBeTruthy();
});
...
test("Simplr Router Links should have updated data attributes after initialization", () => {
    ...
    let oldDataAttributeElements = Array.from(document.body.querySelectorAll("[data-simplr-route]"));
    let initializedDataAttributeElements = Array.from(document.body.querySelectorAll("[data-simplr-route-initialized]"));
...
test ("SetTransitionSpeed should throw on invalid value", () => {
    SimplrRouter.init(mockActiveView(), routes, false);
    expect(() => {SimplrRouter.setTransitionSpeed("abc")}).toThrowError();
});

test ("SetTransitionSpeed should not throw on numeric value", () => {
    SimplrRouter.init(mockActiveView(), routes, false);
    expect(() => {SimplrRouter.setTransitionSpeed(0.2)}).not.toThrowError();
    expect(() => {SimplrRouter.setTransitionSpeed(TransitionSpeed.SLOW)}).not.toThrowError();
    expect(() => {SimplrRouter.setTransitionSpeed("0.5")}).not.toThrowError();
});

test ("SetTransitionDirection should throw on invalid value", () => {
    SimplrRouter.init(mockActiveView(), routes, false);
    expect(() => {SimplrRouter.setTransitionDirection("abc")}).toThrowError();
});
...
test ("Should append Simplr Router wrapper on anchor click", () => {
    ...
    expect(document.body.querySelector(".simplr-router-view-wrapper")).not.toBeNull();
});

test("Forbidden action should be settable", () => {
    SimplrRouter.init(mockActiveView(), routes, false);
    let forbiddenAction = () => {console.log("FORBIDDEN")};
...
test("Not Found action should be settable", () => {
    SimplrRouter.init(mockActiveView(), routes, false);
    let notFoundAction = () => {console.log("NOT FOUND")};
...
test("Router should be able to load styles", () => {
    expect(() => {SimplrRouter.init(mockActiveView(), routes, true);}).not.toThrowError()
});